    ...
    for filepath in files:
        ...
        for i, section in enumerate(sections):
            ...
            if i == 0 and section.startswith("# "):
                ...
                body = "\n".join(lines[1:]).strip()
                if body:
                    chunks.append({"id": f"{filename}_intro", "text": f"# {title}\n\n{body}", "source": filename, "header": title})
            else:
                lines = section.split("\n")
                header = lines[0].strip()
                body = "\n".join(lines[1:]).strip()
                if len(body) > 100:
                    chunks.append({"id": f"{filename}_{i}", "text": f"## {header}\n\n{body}", "source": filename, "header": header})
    return chunks
...
def generate(query, docs, metas):
    context = "\n\n---\n\n".join([f"Source: {m['header']}\n\n{d}" for d, m in zip(docs, metas)])
    system_prompt = "You are a scholarly expert on Tamil and Tamil Nadu history. Answer questions based on the provided context. Be thorough, accurate, and scholarly. Cite specific names, dates, and events."
...
with gr.Blocks(title="Tamil History RAG") as demo:
    ...
    gr.HTML('<div style="text-align:center;padding:1rem 0 0;color:#888;font-size:0.8rem">Corpus: 10 scholarly chapters · Prehistoric → Sangam → Pallava → Chola → Pandya → Nayak → Colonial → Dravidian → Modern</div>')
